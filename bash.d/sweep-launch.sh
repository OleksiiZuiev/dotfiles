# Launch parallel /refine Claude sessions — one Windows Terminal tab per ticket.
#
...
#   - Launches claude with /refine <ticket-id>
...
sweep_launch() {
    ...
    for ticket_id in "$@"; do
        echo "Launching /refine session for: $ticket_id"
        ...
        wt.exe -w 0 new-tab \
            ...
            "$BASH_WIN" -c "\"$CLAUDE_PATH\" \"/refine $ticket_id\""
    done
